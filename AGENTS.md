...
- Include `<a href="../../app-index.html">Back to apps</a>` somewhere in each app's `index.html` so users can return to the actual apps page. The featured mountain-refuge demo has two purpose entrances in one canonical file: `?view=scheduler` returns to `../../petri-smc-wbs.html#app-20`, while `?view=comparison` returns to `../../app-index.html#app-20-comparison`. Retain both explicit collection links and the existing hash routes. Older apps may still use the legacy `../../index.html` return route; preserve them unless their navigation is in scope.
