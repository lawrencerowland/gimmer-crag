...
- Include `<a href="../../app-index.html">Back to apps</a>` somewhere in each app's `index.html` so users can return to the actual apps page. For the featured mountain-refuge demo, return to `../../petri-smc-wbs.html#app-20`. Older apps may still use the legacy `../../index.html` return route; preserve them unless their navigation is in scope.
