...
- `app-index.html`: the eleven other numbered apps in observation/binding, procurement, ontology, sheaf/change and resource scheduling experiments, plus an explicit comparison reference to App 20. The comparison tests why two supplied mechanisms can give the same chosen plan; it does not infer a mechanism from a plan.
...
The numbered primary memberships remain a disjoint 16/11 partition. The manifest records the broader comparison as a reference, so there are 27 unique apps and 28 purpose entries, not a duplicate app or code copy. Both collection return anchors remain explicit in the app.
